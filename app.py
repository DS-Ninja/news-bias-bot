...
# UPDATED v2026-02-10c (FULL REBUILD / FIXED):
# ✅ XSS-safe payload injection (no </script> breakouts)
# ✅ /run POST returns meta-only (lighter + consistent)
# ✅ Token compare uses hmac.compare_digest (constant-time)
# ✅ Calendar inserted counter counts ONLY true inserts (xmax=0 trick)
# ✅ Optional: DB connection pooling (SimpleConnectionPool) to avoid "too many connections"
# ✅ Minor robustness: safer json dumps in HTML, defensive DB init, stable responses
...
def _make_dsn() -> str:
    ...
    # NOTE: keep sslmode separate if user sets PGSSLMODE
...
def _auth_run(token: str) -> bool:
    ...
    t_hash = hashlib.sha256(t.encode()).digest()
    for x in RUN_TOKENS:
        if hmac.compare_digest(t_hash, hashlib.sha256(x.encode()).digest()):
            return True
    return False
