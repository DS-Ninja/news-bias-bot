...
#
# UI overhaul (2026-02-07):
# - Added UI modes: Trader / Analyst / Debug (localStorage)
# - Reduced first-screen overload (Trader mode)
# - Heavy sections moved into <details> collapsibles (perfect for iPhone Safari)
# - Macro window collapses to 1-line when empty
# - Flip metrics redesigned: "To Neutral" and "To Opposite" (no confusing 0.0)
# - GET /run now shows a helpful instruction page (POST required)
#
# Run:
...
# --- NEW: flip metrics in a human way
def flip_metrics(score: float, th: float) -> Dict[str, Any]:
    """
    Units: 'score units' (sum of weighted evidence contributions), NOT % and NOT points.
    Neutral zone is between [-th, +th].
    """
    # Distance to become bullish (reach +th)
    to_bullish = max(0.0, th - score)
    # Distance to become bearish (reach -th)
    to_bearish = max(0.0, score + th)

    # Distance to return to neutral boundary (if already outside neutral)
    # If bullish: need to drop down to +th
    # If bearish: need to rise up to -th
    if score >= th:
    ...
    # Opposite flip distance depends on state:
    # If bullish -> opposite is bearish: need to cross -th (delta down)
    # If bearish -> opposite is bullish: need to cross +th (delta up)
    if state == "BULLISH":
        to_opposite = score + th
        opposite_label = "to_bearish"
    elif state == "BEARISH":
        to_opposite = th - score
        opposite_label = "to_bullish"
    else:
        # neutral: opposite is "either side" (closest threshold)
        to_opposite = min(to_bullish, to_bearish)
        opposite_label = "min(threshold distance)"
...
def eval_trade_gate(asset_obj: Dict[str, Any], event_mode: bool, profile: str) -> Dict[str, Any]:
    ...
    # We keep these for guards (they are meaningful internally)
    ...
    why_short = [x.get("why", "") for x in td[:2] if x.get("why")]  # top-2 for trader view
    if not why_short:
        why_short = ["Insufficient matched evidence (rules)"]

    # pick one strongest "must change"
    must_1 = must_change[:1] if must_change else []
...
# Helpful for browser: GET /run shows instruction page (POST required)
...
def run_help():
    ...
    <body style="font-family:system-ui,-apple-system,Segoe UI,Arial,sans-serif;padding:16px;line-height:1.45">
      ...
      <p>Or use the dashboard button “Run now”.</p>
    </body></html>
    """
...
def diag():
    ...
    # show if DATABASE_URL is present (no secrets)
    info["env"] = {
...
def dashboard():
    ...
    # Macro panel content
    has_upcoming = any(True for _ in upcoming[:1])
    macro_compact_line = f"event_mode={str(event_mode).lower()} • upcoming={len(upcoming[:6])} • recent_macro={str(event.get('recent_macro', False)).lower()}"
    ...
    ev_badge = '<span class="pill pill-warn">⚠️ EVENT MODE: ON</span>' if event_mode else '<span class="pill pill-ok">✅ EVENT MODE: OFF</span>'
    tr_badge = '<span class="pill pill-warn">TRUMP: ON</span>' if (trump_enabled and trump_flag) else ('<span class="pill pill-neutral">TRUMP: quiet</span>' if trump_enabled else '<span class="pill pill-neutral">TRUMP: disabled</span>')
    fred_badge = '<span class="pill pill-neutral">FRED: on</span>' if (meta.get("fred", {}).get("enabled", False)) else '<span class="pill pill-neutral">FRED: off</span>'
    ...
    def render_asset(asset: str) -> str:
        ...
        tip_flip = _tooltip("Flip distances", "To Neutral/To Opposite are distances in score-units (weighted evidence sum), not %/points.")
        tip_gate = _tooltip("Trade Gate", f"Gate = Bias + Quality v2 + Conflict + Event Mode + Flip guard. Profile: {gate_profile}.")

        # Gate lists (short for Trader)
        gate_why = "".join([f"<li>{x}</li>" for x in (gate.get("why", []) or [])[:2]]) or "<li>—</li>"
        gate_need_1 = "".join([f"<li>{x}</li>" for x in (gate.get("must_1", []) or [])[:1]]) or "<li>—</li>"
        gate_need_full = "".join([f"<li>{x}</li>" for x in (gate.get("must_change", []) or [])[:4]]) or "<li>—</li>"

        # Flip numbers
        to_neutral = float(flip.get("to_neutral", 0.0))
        to_opp = float(flip.get("to_opposite", 0.0))
        to_bull = float(flip.get("to_bullish", 0.0))
        to_bear = float(flip.get("to_bearish", 0.0))
        flip_state = str(flip.get("state", bias))

        # Top driver (Trader) + all (Analyst)
        td_top1 = top3[:1]
        td_html_1 = ""
        if td_top1:
            x = td_top1[0]
            td_html_1 = f"""
              <div class="td-row">
                <div><b>1. {x.get('why','')}</b></div>
                <div class="muted tiny">abs={x.get('abs_contrib_sum','')}</div>
              </div>
            """
        else:
            td_html_1 = '<div class="muted">—</div>'
        ...
        for i, x in enumerate(top3[:3], start=1):
            ...
            """
        if not td_html_full:
            td_html_full = '<div class="muted">—</div>'

        # consensus by source table (only meaningful when conflict high)
        cs_rows = ""
        for x in cons_by_src[:10]:
        ...
        # Latest relevant headlines (top 3 + expandable)
        kw = {
        ...
        news_html_top = ""
        for (source, title, link) in top3_news:
            news_html_top += f'<li><a href="{link}" target="_blank" rel="noopener">{title}</a> <span class="muted">[{source}]</span></li>'
        if not news_html_top:
            news_html_top = "<li class='muted'>—</li>"

        news_html_rest = ""
        for (source, title, link) in rest_news:
            news_html_rest += f'<li><a href="{link}" target="_blank" rel="noopener">{title}</a> <span class="muted">[{source}]</span></li>'

        # One-line "what are these units" (keep short)
        flip_explain = "Units: score-sum (weighted evidence). Not %/points."
        ...
        <section class="card" id="card-{asset}">
          <div class="card-head">
            <div class="head-left">
              ...
              <div class="sub muted tiny">
                q2={q2}/100 {tip_q2} • conflict={conflict_index} {tip_conf} • evidence={evc} • sources={div}
              </div>
            </div>
            <div class="actions">
              <div class="btnrow">
          ...
          <!-- TRADER VIEW: only the essentials -->
          <div class="grid3 mode-trader">
            <div class="panel">
              ...
              <div class="gatebox">
                ...
                <div class="muted tiny" style="margin-top:10px;">To become OK (top-1):</div>
                <ul class="mini">{gate_need_1}</ul>
              </div>
            </div>
            ...
            <div class="panel">
              ...
              <div class="muted tiny">fresh: 0-2h={freshness.get("0-2h",0)} • 2-8h={freshness.get("2-8h",0)} • 8-24h={freshness.get("8-24h",0)}</div>
            </div>

            <div class="panel">
              <div class="h3">Flip {tip_flip}</div>
              <div class="flipgrid">
                <div class="flipcard">
                  <div class="muted tiny">To neutral</div>
                  <div class="flipnum">{to_neutral}</div>
                </div>
                <div class="flipcard">
                  <div class="muted tiny">To opposite</div>
                  <div class="flipnum">{to_opp}</div>
                </div>
              </div>
              <div class="muted tiny" style="margin-top:8px;">state={flip_state} • {flip_explain}</div>
              <div class="h3" style="margin-top:12px;">Top driver now</div>
              <div class="td">{td_html_1}</div>
            </div>
          </div>

          <!-- ANALYST VIEW: expandable heavy blocks -->
          <div class="mode-analyst">
            <details class="details" open>
              <summary><b>Analyst details</b> <span class="muted tiny">drivers • flip • gate needs</span></summary>

              <div class="grid3" style="margin-top:12px;">
                <div class="panel">
                  <div class="h3">Trade Gate (full)</div>
                  <div class="muted tiny">To become OK:</div>
                  <ul class="mini">{gate_need_full}</ul>
                </div>
                <div class="panel">
                  <div class="h3">Flip expanded</div>
                  <div class="muted tiny">To bullish: {to_bull} • To bearish: {to_bear}</div>
                  <div class="muted tiny" style="margin-top:6px;">To neutral: {to_neutral} • To opposite: {to_opp}</div>
                  <div class="muted tiny" style="margin-top:8px;">{flip_explain}</div>
                </div>
                <div class="panel">
                  <div class="h3">Top 3 drivers</div>
                  <div class="td">{td_html_full}</div>
                </div>
              </div>
              ...
              <div class="grid2" style="margin-top:12px;">
                <div class="panel">
                  ...
                  <ul class="news">{news_html_top}</ul>
                  {"<details class='details' style='margin-top:10px;'><summary><b>Show more</b> <span class='muted tiny'>(up to 12)</span></summary><ul class='news' style='margin-top:10px;'>"+news_html_rest+"</ul></details>" if news_html_rest else ""}
                </div>

                <div class="panel">
                  <div class="h3">WHY (top 5)</div>
                  <ol class="why">{why_html or "<li class='muted'>—</li>"}</ol>
                </div>
              </div>

              {"<div class='panel' style='margin-top:12px;'><div class='h3'>Consensus by source</div><table class='ctable'><thead><tr><th>Source</th><th>Net</th><th>Abs</th><th>n</th></tr></thead><tbody>"+cs_rows+"</tbody></table></div>" if conflict_index > 0.55 else "<div class='muted tiny' style='margin-top:10px;'>Consensus by source hidden (conflict ≤ 0.55). Switch to Debug or raise conflict to auto-show.</div>"}
    ...
    html = f"""
    <html data-theme="dark" data-mode="trader">
    <head>
      <meta charset="utf-8">
      ...
      <style>
        ...
        .grid2 {{ display:grid; grid-template-columns: 1.1fr 1.1fr; gap:12px; margin-top: 12px; }}
        ...
        /* Modes */
        html[data-mode="trader"] .mode-analyst {{ display:none !important; }}
        html[data-mode="analyst"] .mode-trader {{ display:none !important; }}
        html[data-mode="debug"] .mode-trader {{ display:none !important; }}
    ...
    <body>
      <div class="wrap">
        <div class="top">
          <div>
            ...
            <div class="muted tiny" style="margin-top:6px; display:flex; gap:8px; flex-wrap:wrap;">
              ...
              {tr_badge}
              {fred_badge}
              <a class="pill pill-neutral" href="/diag" target="_blank" rel="noopener">/diag</a>
            </div>
          </div>
        ...
        <div class="mini">
          <div class="mini-inner">
            ...
            <div class="toggles">
              <button class="btn" onclick="setMode('trader')">Trader</button>
              <button class="btn" onclick="setMode('analyst')">Analyst</button>
        ...
        <section class="card">
          <div class="card-head" style="align-items:center;">
            <div style="flex:1;">
              ...
              <div class="muted tiny">{macro_compact_line}</div>
              <div class="mode-analyst" style="margin-top:8px;">
                <div class="muted tiny">Upcoming (next):</div>
                <ul class="news">{up_html}</ul>
              </div>
              <div class="mode-trader muted tiny" style="margin-top:8px;">
                {"Upcoming hidden in Trader mode (switch to Analyst)." if has_upcoming else "No upcoming events (empty)."}
              </div>
            </div>
        ...
        <section class="card mode-analyst">
        ...
        <section class="card mode-debug">
          <div class="h2">Debug</div>
          <div class="muted tiny">Use /diag and /bias?pretty=1 for raw payload.</div>
          <div style="margin-top:10px;">
            <a class="btn" href="/diag" target="_blank" rel="noopener">Open /diag</a>
            <a class="btn" href="/bias?pretty=1" target="_blank" rel="noopener">Open Bias JSON</a>
            <a class="btn" href="/feeds_health" target="_blank" rel="noopener">Open /feeds_health</a>
          </div>
        </section>
      ...
      <script>
        ...
        const eventObj = {json.dumps(event, ensure_ascii=False)};
        ...
        (function initMode(){{
          ...
          const mode = (saved === 'trader' || saved === 'analyst' || saved === 'debug') ? saved : 'trader';
        ...
        function setMode(mode){{
          if(!(mode === 'trader' || mode === 'analyst' || mode === 'debug')) return;
          document.documentElement.setAttribute('data-mode', mode);
        ...
        async function runNow(asset) {{
          ...
          try {{
            const resp = await fetch('/run', {{ method:'POST', headers:{{'Content-Type':'application/json'}}, body:'{}' }});
        ...
        function showEvent() {{
          openModal('Event JSON', '<pre style="white-space:pre-wrap;">' + escapeHtml(JSON.stringify(eventObj, null, 2)) + '</pre>');
        }}
